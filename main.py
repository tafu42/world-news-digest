...
def _gen_prompt(group):
    ...
    return (f"次の{len(group)}件の英語ニュースについて、それぞれ次の3つを日本語で作ってください。\n"
            "【最重要】原文に書かれていない情報を創作・推測で補わないこと（ハルシネーション厳禁）。"
            "原文にない固有名詞・数値・因果関係を足さない。\n"
            "① title: 自然な日本語タイトル。\n"
            "② summary: 本文の要約（2文以内・具体的に）。原文から確実に言えることは"
            "断定形（「〜した」「〜である」）で書く。\n"
            ...
            f"必ず{len(group)}件を、記事の順番どおりにJSON配列で返してください。\n"
