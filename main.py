...
TOP_CATEGORY = 'テクノロジー'           # Guardian technology（並び順の先頭。影響は全記事に付く）
...
# タイトル・要約・社会への影響を1回の呼び出しでまとめて生成する（影響を全記事に付けても
# 同じ呼び出しの中で返るので、Geminiの回数は増えない）。
GEN_SCHEMA = types.Schema(
...
# --- タイトル＋要約＋社会への影響を、1回のリクエストでまとめて生成 ---
# 影響は翻訳と同じ呼び出しの中で返るので、全記事に付けてもGeminiの回数は増えない。
def _gen_prompt(group):
    # 渡せるのは Guardian の trailText（リード文）や Hacker News の og:description ＝
    # 記事の冒頭・概要だけで、本文全体ではない。それを「本文」と呼んで具体性を強く要求すると、
    # 無い情報を創作させることになるので、素材が何かを正直に伝える。
    # プロンプトの順序：【厳守事項】→【指示】→【素材データ】。
    items = "\n\n".join(
        f"[{i+1}]\nTitle: {a['en_title']}\nLead: {a['en_text'][:300]}"
    ...
    return (
        "【厳守】\n"
        "・与えられるのは記事の【リード文（冒頭の概要）だけ】。本文全体ではなく、"
        "途中で切れていることもある。\n"
        "・リード文・タイトルに書かれていない情報を創作・推測で補わないこと"
        "（ハルシネーション厳禁）。書かれていない固有名詞・数値・因果関係を足さない。"
        "途中で切れていても、その続きを推測で補完しないこと。\n"
        "・書かれている範囲だけを使い、無いことは無理に埋めない。\n"
        f"\n【指示】次の{len(group)}件の英語ニュースについて、それぞれ title・summary・impact を"
        "日本語で作る。\n"
        "① title: 自然な日本語タイトル。\n"
        "② summary: リード文の要約（2文以内）。確実に言えることは"
        "断定形（「〜した」「〜である」）で書く。\n"
        "③ impact: このニュースで起こりうる社会への影響（1〜2文）。既に確定した事実は"
        "断定形で書いてよいが、これから起こりうる見通し・予測は「〜と考えられる」"
        "「〜する可能性がある」「〜とみられる」など推測の表現にし、"
        "断定（事実）と推測（予測）をはっきり区別すること。"
        "影響が小さい話題は、無理に大げさにせず簡潔でよい。\n"
        f"必ず{len(group)}件を、記事の順番どおりにJSON配列で返すこと。\n"
        '形式: [{"title": "...", "summary": "...", "impact": "..."}]\n'
        "\n【素材】\n" + items)
...
def _gen_fallback(a):
    ...
    a['_failed'] = True
    return {'title': a['en_title'], 'summary': a.get('en_text', '')[:120], 'impact': ''}
...
def _apply(a, r, now_iso):
    """生成結果（title/summary/impact）を記事へ反映。
    失敗（_failed）なら pending を立て、材料(en_*)を残して次回に備える。"""
    a['title'] = (r or {}).get('title') or a['en_title']
    a['summary'] = (r or {}).get('summary') or ''
    impact = str((r or {}).get('impact') or '').strip()
    if impact:
        a['impact'] = impact
...
def retry_pending(articles, now_iso):
    ...
    if not pend:
        return 0
    changed = 0
    for a, r in zip(pend, _gen_all(pend, _gen_prompt, GEN_SCHEMA, _gen_valid, lambda x: None)):
        if isinstance(r, dict) and looks_translated(r.get('title', '')):
            a['title'] = r['title']
            a['summary'] = r.get('summary') or a.get('summary', '')
            imp = str(r.get('impact') or '').strip()
            if imp:
                a['impact'] = imp
            a.pop('pending', None)
            a.pop('en_title', None)
...
def main():
    ...
    if new_articles:
        ...
        # ① タイトル＋要約＋社会への影響を1回でまとめて生成（失敗分は pending として印を残す）
        for a, r in zip(new_articles, _gen_all(new_articles, _gen_prompt, GEN_SCHEMA,
                                               _gen_valid, _gen_fallback)):
            _apply(a, r, now_iso)
